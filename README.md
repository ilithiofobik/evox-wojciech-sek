...
## Deployment
The API is deployed on Heroku using Heroku Postgresql to store its database. It is available  [here](https://evox-wsek.herokuapp.com
).
...
- /login
  ...
  - **Description**: 
    ...
    This method checks if there is a row with given login and sha512 hash value of the given password in the table accounts of our database.
    
    If so it generates a random token, adds it to the collection of currently used session tokens and sets a cookie with key "session_token" and the value of generated token.
...
- /messages/{message_id}
  ...
  - **Description**: 
    ...
    This method returns `Content` and `Counter` of the message with given `Messageid`. It also increments the `Counter`.
